...
% vehicle in the same lane (lead car), $D_{rel}$. The sensor also measures
% the relative velocity of the lead car, $V_{rel}$. The ACC system operates
% in the following two modes:
%
% * Speed control: The ego car travels at a driver-set speed.
% * Spacing control: The ego car maintains a safe distance from the lead
% car.
%
...
% speed control. In other words, the ACC system makes the ego car travel
...
%
% * If $D_{rel} \geq D_{safe}$, then speed control mode is active. The
% control goal is to track the driver-set velocity, $V_{set}$.
%
% * If $D_{rel} < D_{safe}$, then spacing control mode is active. The
...
% lead car varies according to a sine wave during the simulation. The
% Adaptive Cruise Control System block outputs an acceleration control
% signal for the ego car.
...
% acceleration and velocity are modeled as:
%
% $$ G = \frac{1}{s(0.5s+1)} $$
%
...
% maintain a safe distance to the lead car, the ego car accelerates with a
% slower rate.
%
% From 13 to 25 seconds, the ego car maintains the driver-set velocity, as
...
% again. The ego car maintains a safe distance from the lead car by
% adjusting its speed, as shown in the *Distance* plots.
%
% From 45 to 56 seconds, the spacing error is above |0|. Therefore, the ego
% car achieves the driver-set velocity again.
%
% From 56 to 76 seconds, the deceleration/acceleration sequence from the 25
% to 45 second interval is repeated.

%%
% Throughout the simulation, the controller ensures that the actual
% distance between the two vehicles is greater than the set safe distance.
